...
std::unique_ptr<AST> Parser::parseInline(std::shared_ptr<Scanner> const& _scanner)
{
	...
	try
	{
		ASTNodeRegistryBuilder nodeRegistryBuilder;
		m_scanner = _scanner;
		if (m_useSourceLocationFrom == UseSourceLocationFrom::Comments)
			fetchDebugDataFromComment();
		auto block = parseBlock(nodeRegistryBuilder);
		return std::make_unique<AST>(m_dialect, nodeRegistryBuilder.build(), std::move(block));
	}
...
Block Parser::parseBlock(ASTNodeRegistryBuilder& _nodeRegistryBuilder)
{
	RecursionGuard recursionGuard(*this);
	Block block = createWithDebugData<Block>();
	expectToken(Token::LBrace);
	while (currentToken() != Token::RBrace)
		block.statements.emplace_back(parseStatement(_nodeRegistryBuilder));
	updateLocationEndFrom(block.debugData, currentLocation());
	advance();
	return block;
}

Statement Parser::parseStatement(ASTNodeRegistryBuilder& _nodeRegistryBuilder)
{
	RecursionGuard recursionGuard(*this);
	switch (currentToken())
	{
	case Token::Let:
		return parseVariableDeclaration(_nodeRegistryBuilder);
	case Token::Function:
		return parseFunctionDefinition(_nodeRegistryBuilder);
	case Token::LBrace:
		return parseBlock(_nodeRegistryBuilder);
	case Token::If:
	{
		If _if = createWithDebugData<If>();
		advance();
		_if.condition = std::make_unique<Expression>(parseExpression(_nodeRegistryBuilder));
		_if.body = parseBlock(_nodeRegistryBuilder);
	...
	case Token::Switch:
	{
		...
		advance();
		_switch.expression = std::make_unique<Expression>(parseExpression(_nodeRegistryBuilder));
		while (currentToken() == Token::Case)
			_switch.cases.emplace_back(parseCase(_nodeRegistryBuilder));
	...
	case Token::For:
		return parseForLoop(_nodeRegistryBuilder);
	case Token::Break:
	{
	...
	// Assignment
	std::variant<Literal, Identifier, BuiltinName> elementary(parseLiteralOrIdentifier(_nodeRegistryBuilder));

	switch (currentToken())
	{
	case Token::LParen:
	{
		Expression expr = parseCall(_nodeRegistryBuilder, std::move(elementary));
	...
	case Token::AssemblyAssign:
	{
			...
			std::visit(GenericVisitor{
				...
				[&](Identifier const& _identifier)
				{
					...
					else
					{
						...
						elementary = parseLiteralOrIdentifier(_nodeRegistryBuilder);
					}
				}
			}, elementary);
		...
		assignment.value = std::make_unique<Expression>(parseExpression(_nodeRegistryBuilder));
...
Case Parser::parseCase(ASTNodeRegistryBuilder& _nodeRegistryBuilder)
{
	RecursionGuard recursionGuard(*this);
	...
	{
		advance();
		std::variant<Literal, Identifier, BuiltinName> literal = parseLiteralOrIdentifier(_nodeRegistryBuilder);
	...
	_case.body = parseBlock(_nodeRegistryBuilder);
	updateLocationEndFrom(_case.debugData, nativeLocationOf(_case.body));
	return _case;
}

ForLoop Parser::parseForLoop(ASTNodeRegistryBuilder& _nodeRegistryBuilder)
{
	RecursionGuard recursionGuard(*this);
	...
	forLoop.pre = parseBlock(_nodeRegistryBuilder);
	m_currentForLoopComponent = ForLoopComponent::None;
	forLoop.condition = std::make_unique<Expression>(parseExpression(_nodeRegistryBuilder));
	m_currentForLoopComponent = ForLoopComponent::ForLoopPost;
	forLoop.post = parseBlock(_nodeRegistryBuilder);
	m_currentForLoopComponent = ForLoopComponent::ForLoopBody;
	forLoop.body = parseBlock(_nodeRegistryBuilder);
...
Expression Parser::parseExpression(ASTNodeRegistryBuilder& _nodeRegistryBuilder, bool const _unlimitedLiteralArgument)
{
	RecursionGuard recursionGuard(*this);

	std::variant<Literal, Identifier, BuiltinName> operation = parseLiteralOrIdentifier(
		_nodeRegistryBuilder,
	...
	return visit(GenericVisitor{
		[&](Identifier& _identifier) -> Expression
		{
			if (currentToken() == Token::LParen)
				return parseCall(_nodeRegistryBuilder, std::move(operation));
...
)
{
	...
	case Token::Identifier:
	{
		...
		if (std::optional<BuiltinHandle> const builtinHandle = m_dialect.findBuiltin(currentLiteral()))
			...
			literalOrIdentifier = Identifier{createDebugData(), _nodeRegistryBuilder.define(currentLiteral())};
		advance();
	...
	case Token::FalseLiteral:
	{
		...
		if (currentToken() == Token::Colon)
		{
			...
			std::ignore = expectAsmIdentifier(_nodeRegistryBuilder);
...
VariableDeclaration Parser::parseVariableDeclaration(ASTNodeRegistryBuilder& _nodeRegistryBuilder)
{
	RecursionGuard recursionGuard(*this);
	...
	while (true)
	{
		varDecl.variables.emplace_back(parseNameWithDebugData(_nodeRegistryBuilder));
		if (currentToken() == Token::Comma)
			expectToken(Token::Comma);
		else
			break;
	...
	{
		expectToken(Token::AssemblyAssign);
		varDecl.value = std::make_unique<Expression>(parseExpression(_nodeRegistryBuilder));
...
FunctionDefinition Parser::parseFunctionDefinition(ASTNodeRegistryBuilder& _nodeRegistryBuilder)
{
	RecursionGuard recursionGuard(*this);
	...
	funDef.name = expectAsmIdentifier(_nodeRegistryBuilder);
	expectToken(Token::LParen);
	while (currentToken() != Token::RParen)
	{
		funDef.parameters.emplace_back(parseNameWithDebugData(_nodeRegistryBuilder));
	...
	if (currentToken() == Token::RightArrow)
	{
		...
		while (true)
		{
			funDef.returnVariables.emplace_back(parseNameWithDebugData(_nodeRegistryBuilder));
	...
	funDef.body = parseBlock(_nodeRegistryBuilder);
...
NameWithDebugData Parser::parseNameWithDebugData(ASTNodeRegistryBuilder& _nodeRegistryBuilder)
{
	RecursionGuard recursionGuard(*this);
	NameWithDebugData typedName = createWithDebugData<NameWithDebugData>();
	auto const nameLocation = currentLocation();
	typedName.name = expectAsmIdentifier(_nodeRegistryBuilder);
	if (currentToken() == Token::Colon)
	{
		expectToken(Token::Colon);
...
YulName Parser::expectAsmIdentifier(ASTNodeRegistryBuilder& _nodeRegistryBuilder)
{
	...
	auto const name = _nodeRegistryBuilder.define(identifier);
