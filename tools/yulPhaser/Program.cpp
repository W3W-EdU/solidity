...
Program::Program(Program const& program):
	m_ast(std::make_unique<AST>(program.m_dialect, ASTNodeRegistry{}, std::get<Block>(ASTCopier{}(program.m_ast->root())))),
...
std::variant<std::unique_ptr<AST>, ErrorList> Program::parseObject(Dialect const& _dialect, CharStream _source)
{
	...
	auto astCopy = std::make_unique<AST>(_dialect, ASTNodeRegistry{}, std::get<Block>(ASTCopier{}(selectedObject->code()->root())));
...
)
{
	...
	return std::make_unique<AST>(_dialect, ASTNodeRegistry{}, std::get<Block>(disambiguator(_ast.root())));
}
...
)
{
	...
	return std::make_unique<AST>(_dialect, ASTNodeRegistry{}, std::move(astRoot));
}
