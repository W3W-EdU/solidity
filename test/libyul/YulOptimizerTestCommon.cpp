	...
	m_namedSteps = {
		...
		{"stackCompressor", [&]() {
			...
			size_t maxIterations = 16;
			{
				...
				object.setCode(std::make_shared<AST>(*m_object->dialect(), ASTNodeRegistry{}, std::get<Block>(ASTCopier{}(block))));
...
bool YulOptimizerTestCommon::runStep()
{
	if (m_namedSteps.count(m_optimizerStep))
	{
		...
		m_optimizedObject->setCode(std::make_shared<AST>(*m_object->dialect(), ASTNodeRegistry{}, std::move(block)));
	}
	else
		return false;
